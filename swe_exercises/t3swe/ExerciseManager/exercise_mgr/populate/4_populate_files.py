...
def write_all_semester(conn):
    with open(Path(__file__).parent / 'topics.json', 'r', encoding='utf8') as topics_json:
        topics = json.load(topics_json)
    directories = [Settings.files / str(i) / "split" for i in range(12, 20)]
    ...
    for path in paths:
        if path.stem in topics:
            exercise_texts[path.stem] = [path, s_ps_en_from_string(path.stem), topics[path.stem][2]]
        for topic in topics:
            if path.stem in topics[topic][0]:
                exercise_texts[path.stem] = [path, s_ps_en_from_string(path.stem), topics[topic][2]]
    ...
    for problem_set in problem_sets:
        ps = s.problem_set_insert(conn, (None, None, None, 1, problem_set[1], problem_set[0]))
    ...
    for exercise_text in exercise_texts:
        ...
        et = s.exercise_text_insert(conn, (None, exercise_texts[exercise_text][2], exercise_text, content))
        ps = -1
        for problem_set in problem_sets:
            if problem_set[0:2] == exercise_texts[exercise_text][1][0:2]:
                ps = problem_set[2]
        s.exercise_to_problem_set_insert(conn, (et, ps, exercise_texts[exercise_text][1][2], points))
